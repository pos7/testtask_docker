...
Все другие порты, кроме 8080, не имеют доступа "наружу", если потребуются порты для подключения к админке RabbitMQ и БД monogodb, то нужно раскоментировать соответствующие строки в ```docker-compose.yml``` и перезапустить через ```start.sh```
